...
import com.nmg.teach.common.constant.state.ManagerStatus;
...
import com.nmg.teach.core.shiro.ShiroUser;
import com.nmg.teach.core.util.ToolUtil;
import com.nmg.teach.modular.system.dao.UserMgrDao;
import com.nmg.teach.modular.system.factory.UserFactory;
import com.nmg.teach.modular.system.transfer.UserDto;
import com.nmg.teach.modular.system.warpper.UserWarpper;
...
public class UserMgrController extends BaseController {
    ...
    @Resource
    private TeachProperties teachProperties;
    ...
    public Object changePwd(@RequestParam String oldPwd, @RequestParam String newPwd, @RequestParam String rePwd) {
        if (!newPwd.equals(rePwd)) {
            throw new TeachException(BizExceptionEnum.TWO_PWD_NOT_MATCH);
        }
        Integer userId = ShiroKit.getUser().getId();
        ...
        if (user.getPassword().equals(oldMd5)) {
            ...
            throw new TeachException(BizExceptionEnum.OLD_PWD_NOT_RIGHT);
        }
    }
    ...
    public Tip add(@Valid UserDto user, BindingResult result) {
        ...
        if (theUser != null) {
            throw new TeachException(BizExceptionEnum.USER_ALREADY_REG);
        }
    ...
    public Tip edit(@Valid UserDto user, BindingResult result) throws NoPermissionException {
        ...
        if (ShiroKit.hasRole(Const.ADMIN_NAME)) {
            ...
            if (shiroUser.getId().equals(user.getId())) {
                ...
                throw new TeachException(BizExceptionEnum.NO_PERMITION);
            }
        }
    }
    ...
    public Tip delete(@RequestParam Integer userId) {
        ...
        if (userId.equals(Const.ADMIN_ID)) {
            throw new TeachException(BizExceptionEnum.CANT_DELETE_ADMIN);
        }
        assertAuth(userId);
    ...
    public Tip freeze(@RequestParam Integer userId) {
        ...
        //不能冻结超级管理员
        if (userId.equals(Const.ADMIN_ID)) {
            throw new TeachException(BizExceptionEnum.CANT_FREEZE_ADMIN);
        }
        assertAuth(userId);
    ...
    public Tip setRole(@RequestParam("userId") Integer userId, @RequestParam("roleIds") String roleIds) {
        ...
        //不能修改超级管理员
        if (userId.equals(Const.ADMIN_ID)) {
            throw new TeachException(BizExceptionEnum.CANT_CHANGE_ADMIN);
        }
        assertAuth(userId);
    ...
    public String upload(@RequestPart("file") MultipartFile picture) {
        ...
        try {
            String fileSavePath = teachProperties.getFileUploadPath();
            picture.transferTo(new File(fileSavePath + pictureName));
        } catch (Exception e) {
            throw new TeachException(BizExceptionEnum.UPLOAD_ERROR);
        }
        return pictureName;
    }
